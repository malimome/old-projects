...
Begin VB.Form frmGraphFuncF 
   ...
   End
End
Attribute VB_Name = "frmGraphFuncF"
...
On Error GoTo er
test = False
If BN(1) = 1 Then GoTo er
test = True
er:
If test = False Then Command3_Click
...
End Sub
Private Function Integral(t As Long, blnCos As Boolean) As Double
    Dim i As Double
    Dim Ret As Double
...
Dim Upper As Integer, n As Long
Expr = Text1
...
Upper = Val(txtn2)
    ...
    AN(0) = 1 / PI * Integral(0, True)
    For n = 1 To Upper
        AN(n) = 1 / PI * Integral(n, True)
        BN(n) = 1 / PI * Integral(n, False)
    Next n
...
For i = 1 To Val(txtn2)
    Ret = Ret + AN(i) * Cos(i * X) + BN(i) * Sin(i * X)
Next i
Ret = Ret + AN(0)
FN = Ret
End Function
